...
// DDFC安排
class DdfcEmpCheck extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            confirmDirty: false,
...
// 根据工作包分线查询符合条件的员工列表提交
    ManageWorkDaySubmit = (e) => {
        e.preventDefault();
        const value=this.props.empFind;
        // console.log('value',value);
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                // console.log('Received values of form: ', values);
                Api.post('employeeInfo/findEmployeeByConditionNoPage',{
...
//
    componentDidMount(){
        const value=this.props.empFind1;
        Api.post('employeeInfo/findEmployeeByConditionNoPage',{
        }).then(res=>{
            // console.log('被选中',res);
            // 被选中员工
...
// 监听人员是否被选中
    ...
    save = () => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            //    人员类型
            Api.post('deferInfo/ddfcNrcArrange',{
                'stlId':'',
                'receiveState':'T',
                'wwpId':value.wwpId,
                'receiveType':'DDFC',
...
const DdfcEmpCheckForm = Form.create()(DdfcEmpCheck);
export default DdfcEmpCheckForm;
