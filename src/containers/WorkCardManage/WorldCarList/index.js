...
class WorkCardLists extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            loading:true,
...
changeListState = (e,record,value) => {
        if(value && value!==record.listState){
...
//将后台返回的时间戳转化为标准的时间格式
    ...
    // 更新页面数据
    update(){
        const value=this.props.taskAdd;
        console.log('value',value);
        Api.post('workPackageInfo/findSubTaskListByCondition',{
            'commandNo':value?value:null,
            'subTaskNo':'',
            'listState':'',
...
const WorkCardList = Form.create()(WorkCardLists);
