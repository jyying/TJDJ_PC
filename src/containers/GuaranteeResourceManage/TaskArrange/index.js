...
// 工卡清单安排
class TaskArrange extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
...
// 工卡明细
        this.expandedRowRender = () => {
            ...
            return (
                <div>
                    ...
                    {/*/>*/}
                    <Pagination showQuickJumper showSizeChanger  defaultCurrent={this.state.currentPage1} total={this.state.totalSize1} onChange={this.onChange1}  showTotal={total => `合计 ${total} 条` }
    ...
    onShowSizeChange=(current, pageSize)=>{
        ...
        Api.post('weekWorkPackageTask/findSubTaskListByWwpId',{
            wwpId:this.state.wwpId,
            wwpaId:'',arrangedState:this.state.arrangedState!=''?this.state.arrangedState:'',
            pageNow:1,
            pageSize:pageSize
        }).then(res => {
                // console.log('res',res);
                if(res.errorCode == 0) {
                    this.setState({
                        ...
                        tableLoad:false,
                        // page1:res.pageInfo,
                        currentPage1:parseInt(res.pageInfo.currentPage),
                        totalSize1:parseInt(res.pageInfo.totalSize),
                        pageSize:pageSize
                    });
                }
...
// 分页查询
    onChange1 = (pageNumber) => {
        Api.post('weekWorkPackageTask/findSubTaskListByWwpId',{
            wwpId:this.state.wwpId,wwpaId:'',
            arrangedState:this.state.arrangedState!=''?this.state.arrangedState:'',
            pageNow:pageNumber,
...
// 经理根据工作包分线查询符合条件的生产线经理modal
 ...
 // 创建周计划modal
  ...
  // 更新页面数据
    update(){
        this.props.form.validateFields(['commandNo','executeTime','pageNow','packageStatus','airplaneModel','airplaneRegNo'],(err, values) => {
...
//     };
    ...
    render(){
        ...
        const {data , tableLoading,WorkPlansCheck, DayPlanVFindisible, MouldSelectModal, details, establish, isUpdate, page,pageNow, Selected, managerEmpCheck,WorkCardPNVisible,children,NRCcheckVisible,DDFCcheckVisible} = this.state;
        const upload = {
                customRequest:(obj) => {
                    this.setState({
                        loadings:true
                    ...
                    Api.put('fileUpload/fileUploadCommonPost',{fileName:obj.file})
                        .then(res => {
                            ...
                            if(res.errorCode == 0) {
                                Api.post('weekWorkPackage/batchImport',{importFileName:res.data})
                                    .then(res => {
                                        if(res.errorCode == 0) {
                                            this.setState({
                                                loadings:false
...
const TaskArranges = Form.create()(TaskArrange);
export default TaskArranges;
