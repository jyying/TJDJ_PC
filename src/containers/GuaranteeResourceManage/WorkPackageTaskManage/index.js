...
import ProjectChangeList from '../../GuaranteeResourceManage/ProjectChangeList';
...
// 工作包工卡管理
class WorkPackageTaskManage extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
                ...
                return (
                    // record.battleLineName?(
                        <span className="action">
                             <a onClick={()=>this.managerEmpCheck(record)}>跟线员分配</a>
                            ...
                            {/*<Popconfirm title="确认要删除此条信息吗?" onConfirm={()=>this.delete(record)} onCancel={this.cancel} okText="确认" cancelText="取消"><a style={{color:'#e60012'}}>删除</a></Popconfirm>*/}
                            {
                                this.state.hover == record.id ? (
                                    <span className="a">
                                    <span className="ant-divider"/>
                                       <a onClick={() => this.Jobcard(record)}>工卡变更清单</a>
                                         <span className="ant-divider"/>
...
// 工卡明细
    ...
    // 工卡变更清单
    Jobcard= (record) => {
        let Jobcardchecks = false;
        if(record.id) {
            Jobcardchecks = record;
        }
        this.setState({
            Jobcardchecks:Jobcardchecks,
            JobcardchecksVisible: true
        });
    };
    JobcardCancel = () => {
        this.setState({
...
//     };
    ...
    render(){
        ...
        const {data , tableLoading,WorkPlansCheck, DayPlanVFindisible, MouldSelectModal, details, establish, isUpdate, page,pageNow, Selected, managerEmpCheck,WorkCardPNVisible,children,NRCcheckVisible,DDFCcheckVisible,JobcardchecksVisible} = this.state;
        const upload = {
                customRequest:(obj) => {
                    this.setState({
                        loadings:true
            ...
            <div>
                ...
                <div className="content">
                    <div style={{width:'100%'}}>
                        ...
                        <Modal
                            title=''
                            visible={JobcardchecksVisible}
                            onCancel={this.JobcardCancel}
                            onOk={this.handleOk}
                            maskClosable={false}
                            footer={null}
                            key={`${modalKey}keya`}
                            width="80%"
                        >
                            <ProjectChangeList Jobcardchecks={this.state.Jobcardchecks}/>
                        </Modal>
                    </div>
...
const WorkPackageTaskManages = Form.create()(WorkPackageTaskManage);
export default WorkPackageTaskManages;
